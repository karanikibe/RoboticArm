...
public class lowerJointRotation : MonoBehaviour
{
    ...
    void StartServer()
    {
        try
        {
            server = new TcpListener(IPAddress.Parse("127.0.0.1"), 10000);
            server.Start();
            Debug.Log("Unity TCP server started on port 10000...");
    ...
    void Update()
    {
        ...
        {
            joint.localEulerAngles = new Vector3(0f,receivedAngle, 0f); // Only change Y, fix X and Z
